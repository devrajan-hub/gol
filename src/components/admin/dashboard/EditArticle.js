...
const EditArticle = (props) => {
    ...
    const articleId = props.location.state.article.id;
    const articletitle = props.location.state.article.title;
    const articletext = props.location.state.article.text;
    const articledate = props.location.state.article.createdOn;
    const articleimg = props.location.state.article.coverImageURL;
    ...
    const updateArticle = async(e) => {
        ...
        formData.append("articleDetails", newarticledata);
        const result = await fetch('https://viddey-backend.herokuapp.com/api/v1/articles/'+articleId ,{
            method : "PUT",
            headers:{
                        ...
                        <div className="article-resources">
                            <form method="post" action="">
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="text" className="form-control" id="" placeholder="Title" value={articletitle} onChange={handleInput} />
                                </div>
                                <div className="form-group">
                                    <label for="">3. Text</label>
                                    <textarea name="txtmessage" className="form-control" rows="15" cols="3" type="text"  onChange={handleTextArea} placeholder="Text">{articletext}</textarea>
                                </div>
                                <button type="submit" className="btn btn-primary btn-gradient" onClick={updateArticle}>Update and Publish</button>
                            </form>
                        </div>
                    </div>
                </div>
